...
extern "C" {
 ...
 static void _hoc_hillfunction(void);
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "hillfunction_sk", _hoc_hillfunction,
 0, 0
};
#define hillfunction hillfunction_sk
 extern double hillfunction( double );
...
_MATELM1(0, 0) *= ( diam * diam * PI / 4.0);  }
 ...
 f_flux = b_flux = 0.;
 _RHS1( 0) += (b_flux =   ( - ik * diam * PI * ( 1e4 ) / FARADAY ) );
 /*FLUX*/
    } return _reset;
 }
 
double hillfunction (  double _lci ) {
   double _lhillfunction;
 _lhillfunction = gkbar / ( 1.0 + pow( ( kd / _lci ) , hill ) ) ;
   
return _lhillfunction;
 }
 
static void _hoc_hillfunction(void) {
  double _r;
   _r =  hillfunction (  *getarg(1) );
 hoc_retpushx(_r);
}
...
static void _ode_matsol_instance1(_threadargsproto_) {
 _cvode_sparse(&_cvsparseobj1, 1, _dlist1, _p, _ode_matsol1, &_coef1);
 }
...
/*VERBATIM*/
	cai = _ion_cai;
 gkca = hillfunction ( _threadargscomma_ cai ) ;
   ik = gkca * ( v - ek ) ;
   qk = 0.0 ;
   }
 ...
 v=_v;
{
  cai = _ion_cai;
  ek = _ion_ek;
 { error = sparse(&_sparseobj1, 1, _slist1, _dlist1, _p, &t, dt, kstate,&_coef1, _linmat1);
 if(error){fprintf(stderr,"at line 57 in file sk.mod:\n	SOLVE kstate METHOD sparse\n"); nrn_complain(_p); abort_run(error);}
    if (secondorder) {
    int _i;
    for (_i = 0; _i < 1; ++_i) {
