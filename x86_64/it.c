...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "ghk_cat", _hoc_ghk,
 "h_inf_cat", _hoc_h_inf,
 "m_inf_cat", _hoc_m_inf,
 "tau_m_cat", _hoc_tau_m,
 0, 0
};
#define ghk ghk_cat
#define h_inf h_inf_cat
#define m_inf m_inf_cat
#define tau_m tau_m_cat
 ...
 extern double tau_m( _threadargsprotocomma_ double );
 /* declare global and static user variables */
#define shifth shifth_cat
 double shifth = 20;
#define shiftm shiftm_cat
 double shiftm = 20;
#define tauh tauh_cat
 ...
 "shiftm_cat", &shiftm_cat,
 "shifth_cat", &shifth_cat,
 "tauh_cat", &tauh_cat,
 0,0
};
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   Dm = ( m_inf ( _threadargscomma_ v ) - m ) / tau_m ( _threadargscomma_ v ) ;
   Dh = ( h_inf ( _threadargscomma_ v ) - h ) / tauh ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 Dm = Dm  / (1. - dt*( ( ( ( - 1.0 ) ) ) / tau_m ( _threadargscomma_ v ) )) ;
 Dh = Dh  / (1. - dt*( ( ( ( - 1.0 ) ) ) / tauh )) ;
  return 0;
}
 /*END CVODE*/
 static int state (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
    m = m + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tau_m ( _threadargscomma_ v ))))*(- ( ( ( m_inf ( _threadargscomma_ v ) ) ) / tau_m ( _threadargscomma_ v ) ) / ( ( ( ( - 1.0 ) ) ) / tau_m ( _threadargscomma_ v ) ) - m) ;
    h = h + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tauh)))*(- ( ( ( h_inf ( _threadargscomma_ v ) ) ) / tauh ) / ( ( ( ( - 1.0 ) ) ) / tauh ) - h) ;
   }
  return 0;
}
...
double tau_m ( _threadargsprotocomma_ double _lv ) {
   double _ltau_m;
 _ltau_m = ( 1.0 / ( exp ( ( _lv - shiftm + 131.6 ) / - 16.7 ) + exp ( ( _lv - shiftm + 16.8 ) / 18.2 ) ) + 0.612 ) / tadjm ;
...
static void _hoc_tau_m(void) {
  ...
  _thread = _extcall_thread;
  _nt = nrn_threads;
 _r =  tau_m ( _p, _ppvar, _thread, _nt, *getarg(1) );
 hoc_retpushx(_r);
}
 
double m_inf ( _threadargsprotocomma_ double _lv ) {
   double _lm_inf;
 _lm_inf = 1.0 / ( 1.0 + exp ( ( _lv - shiftm + 60.5 ) / - 6.2 ) ) ;
...
double h_inf ( _threadargsprotocomma_ double _lv ) {
   double _lh_inf;
 _lh_inf = 1.0 / ( 1.0 + exp ( ( _lv - shifth + 84.0 ) / 4.03 ) ) ;
...
static void initmodel(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
  ...
  m = m0;
 {
   ...
   m = m_inf ( _threadargscomma_ v ) ;
   h = h_inf ( _threadargscomma_ v ) ;
   }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   ica = gcatbar * m * m * h * ghk ( _threadargscomma_ v , cai , cao , 2.0 ) ;
   }
 _current += ica;
